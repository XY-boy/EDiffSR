...
class NAFBlock(nn.Module):
    def __init__(self, c, time_emb_dim=None, DW_Expand=2, FFN_Expand=2, drop_out_rate=0.):
        ...
        # multi-scale feature extraction 3×3 5×5 7×7
        self.conv2_3 = nn.Conv2d(in_channels=dw_channel, out_channels=dw_channel, kernel_size=3, padding=1, stride=1,
                                 groups=dw_channel,
                                 bias=True)
...
class ConditionalNAFNet(nn.Module):
    ...
    def __init__(self, img_channel=3, width=16, middle_blk_num=1, enc_blk_nums=[], dec_blk_nums=[], upscale=1):
        ...
        self.enhance = ResidualGroup(num_feat=width, num_block=5)
